...
public class TestController_4Attribute {
    // 생성자 주입 : 생성자에서 같은 타입의 bean 을 가져와 초기화
    private CustomerMapper customerMapper;
    

    @GetMapping("/customer/list")
    public String list(Model model) {       // 메소드 인자인 Model 타입 bean 자동 주입됩니다.
        model.addAttribute("page", 100);    // 출력되면 문자열로 변경
        model.addAttribute("dto",
        TestDto.builder()
            .name("길동GD")
            .address("서울").build());
        model.addAttribute("list", List.of("twice", "아이브", "newjeans", "소녀시대"));
        return "customer/list";             // templte 폴더 아래 customer 폴더에 list.html 저장
    }
